...
void main() {
  group('Extension validation tests', () {
    test('hasExtension returns false for incompatible extensions', () {
      ...
      final anotherExtensionForTestAllExtensions = Unittest.optionalStringExtension;
      ...
      expect(testAllExtensions.hasExtension(extensionForTestAllExtensions), isFalse,
          reason: 'Extension applies to message type but field is not set');
      expect(testAllExtensions.hasExtension(anotherExtensionForTestAllExtensions), isFalse,
          reason: 'Another compatible extension that is not set');

      // Invalid cases - extension does not apply to message type
      expect(testAllExtensions.hasExtension(extensionForOuter), isFalse,
          reason: 'Extension for different message type should return false');
      expect(outer.hasExtension(extensionForTestAllExtensions), isFalse,
          reason: 'Extension for different message type should return false');
      expect(outer.hasExtension(anotherExtensionForTestAllExtensions), isFalse,
          reason: 'Extension for different message type should return false');

      // TestAllTypes doesn't support extensions at all
      expect(testAllTypes.hasExtension(extensionForTestAllExtensions), isFalse,
          reason: 'Message that does not support extensions should return false');
    });

    test('hasExtension returns true only when extension is set and compatible', () {
      final testAllExtensions = TestAllExtensions();
      final outer = Outer();

      // Extensions for different message types
      final extensionForTestAllExtensions = Unittest.optionalInt32Extension;
      final extensionForOuter = Extend_unittest.extensionInner; // This extends Outer
      ...
      expect(testAllExtensions.hasExtension(extensionForTestAllExtensions), isTrue,
          reason: 'Extension is set and compatible');
      expect(outer.hasExtension(extensionForOuter), isTrue,
          reason: 'Extension is set and compatible');
    ...
    test('hasExtension validates extension applicability before checking field presence', () {
      final testAllExtensions = TestAllExtensions();
      
      // Set up extensions with different applicability
      final compatibleExtension = Unittest.optionalInt32Extension;
      final incompatibleExtension = Extend_unittest.extensionInner; // This extends Outer, not TestAllExtensions

      // Verify they have different extendee types
      expect(compatibleExtension.extendee, 'protobuf_unittest.TestAllExtensions',
          reason: 'Compatible extension should extend TestAllExtensions');
      expect(incompatibleExtension.extendee, 'extend_unittest.Outer',
      ...
      expect(testAllExtensions.hasExtension(compatibleExtension), isTrue,
      ...
      expect(testAllExtensions.hasExtension(incompatibleExtension), isFalse,
          reason: 'Incompatible extension should return false even with potential field number collision');
    });

    test('hasExtension works correctly with multiple extensions on same message', () {
      final testAllExtensions = TestAllExtensions();
      ...
      testAllExtensions.setExtension(messageExtension, TestAllTypes_NestedMessage()..i = 99);
      expect(testAllExtensions.hasExtension(intExtension), isTrue);
      expect(testAllExtensions.hasExtension(stringExtension), isTrue);
    ...
    test('hasExtension consistency with getExtension and setExtension error behavior', () {
      final testAllTypes = TestAllTypes(); // Does not support extensions
      ...
      expect(() => testAllTypes.getExtension(incompatibleExtension), throwsArgumentError);

      // setExtension should throw for incompatible extensions
      expect(() => testAllTypes.setExtension(incompatibleExtension, 42), throwsArgumentError);
    });
  });

  group('Extension field number collision regression tests', () {
    test('extensions with same field number on different message types handled correctly', () {
      ...
      final extForOuter = Extend_unittest.extensionInner; // This extends Outer
      ...
      expect(testAllExtensions.hasExtension(extForOuter), isFalse,
          reason: 'TestAllExtensions should not report extension for different message type');
      ...
      expect(outer.hasExtension(extForTestAll), isFalse,
          reason: 'Outer should not report extension for different message type');
